...
#![cfg(all(feature = "sync", feature = "http1_1", feature = "__c_openssl"))]
...
use ylong_http_client::sync_impl::{Body, HttpBody, Response};
use ylong_http_client::{
    EmptyBody, HttpClientError, Proxy, Request, TlsConfig, TlsFileType, TlsVersion,
};
...
fn collect_response_body(mut response: Response<HttpBody>) -> Result<Vec<u8>, HttpClientError> {
    let mut out = Vec::new();
    let mut buf = [0u8; 256];
    loop {
        let n = response.body_mut().data(&mut buf)?;
        if n == 0 {
            break;
        }
...
fn https_proxy(addr: &str, proxy_tls: TlsConfig) -> Result<Proxy, HttpClientError> {
    Proxy::all(format!("https://{addr}").as_str())
...
fn sdv_sync_http_target_over_https_proxy() {
    ...
    let client = ylong_http_client::sync_impl::Client::builder()
        .proxy(https_proxy(&proxy.addr, proxy_tls_config(false).unwrap()).unwrap())
        .build()
        .unwrap();
...
#[test]
fn sdv_sync_https_proxy_mtls_success() {
    let proxy = start_proxy(|listener| {
    ...
    let client = ylong_http_client::sync_impl::Client::builder()
        .proxy(https_proxy(&proxy.addr, proxy_tls_config(true).unwrap()).unwrap())
        .build()
        .unwrap();
    let request = Request::builder()
        .method("GET")
...
#[test]
fn sdv_sync_https_proxy_mtls_missing_client_cert_fails() {
    let proxy = start_proxy(|listener| {
...
#[test]
fn sdv_sync_https_proxy_hostname_mismatch_fails() {
    let proxy = start_proxy(|listener| {
    ...
    let client = ylong_http_client::sync_impl::Client::builder()
        .proxy(https_proxy(&proxy.addr, proxy_tls).unwrap())
        .build()
        .unwrap();
    let request = Request::builder()
        .method("GET")
...
#[test]
fn sdv_sync_https_proxy_wrong_client_cert_fails() {
    let proxy = start_proxy(|listener| {
        let _ = accept_proxy_tls(listener, true)?;
        Ok(())
    });
...
#[test]
fn sdv_sync_https_proxy_client_cert_key_mismatch_fails() {
...
#[test]
fn sdv_sync_https_proxy_tls_version_mismatch_fails() {
    let proxy = start_proxy(|listener| {
...
#[test]
fn sdv_sync_https_proxy_tls_cipher_mismatch_fails() {
    let proxy = start_proxy(|listener| {
        let acceptor = tls_acceptor_config(
            false,
            CERT,
...
#[test]
fn sdv_sync_insecure_proxy_verify_does_not_disable_origin_verify() {
    let proxy = start_proxy(|listener| {
...
#[test]
fn sdv_sync_https_proxy_untrusted_ca_fails() {
    let proxy = start_proxy(|listener| {
        let _ = accept_proxy_tls(listener, false)?;
        Ok(())
    });
